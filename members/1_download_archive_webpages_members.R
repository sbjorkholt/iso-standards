...
walk2(urls, ids, function(link, id) { # Download the files to a local folder
  tryCatch({
    download.file(link, destfile = paste0("../../data/archive/version1/", id, ".htm"), quiet = TRUE)
    Sys.sleep(1)
...
walk2(urls, ids, function(link, id) {
  tryCatch({
    download.file(link, destfile = paste0("../../data/archive/version2/", id, ".htm"), quiet = TRUE)
    Sys.sleep(1)
  }, error=function(e){message("Problematic page, skipped ", id)})
})
...
walk2(urls, ids, function(link, id) {
  tryCatch({
    download.file(link, destfile = paste0("../../data/archive/version3/", id, ".htm"), quiet = TRUE)
    Sys.sleep(1)
  }, error=function(e){message("Problematic page, skipped ", id)})
})
...
walk2(urls, ids, function(link, id) {
  tryCatch({
    download.file(link, destfile = paste0("../../data/archive/version4/", id, ".htm"), quiet = TRUE)
    Sys.sleep(1)
  }, error=function(e){message("Problematic page, skipped ", id)})
})
...
saveRDS(extra_links, file = "../../data/archive/extralinks_version4.rds")
...
for (i in 1:length(urls)) {
  ...
  links[[i]] <- read_html(urls[i]) %>%
    html_elements("section > div > div > div > ul > li > a") %>%
    html_attr("href") %>%
    str_c("https://www.iso.org", .)
...
membership <- urls %>% str_extract("=[A-Z]+") %>% str_remove("=")
member <- urls %>% str_extract("member\\/[0-9]+") %>% str_remove("member\\/")
ids <- str_c(member, "_", "2022-08-31", "_", membership)

pages <- tibble(urls, ids) %>% na.omit() 

walk2(pages$urls, pages$ids, function(link, id) {
  tryCatch({
    download.file(link, destfile = paste0("../../data/archive/current/", id, ".htm"), quiet = TRUE)
    Sys.sleep(1)
  }, error=function(e){message("Problematic page, skipped ", id)})
})
